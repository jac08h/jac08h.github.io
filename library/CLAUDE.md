...
Reads `/books/quotes/data/quotes.json` (fetched at runtime, absolute path),
generated by `scripts/build_quotes_json.py` from the repo root. Each book:
`id, author, title, year, url, quotes[]`. Quotes vary wildly in length (a few
chars up to ~4000) and may contain `\n` — the overlay must handle both
...
rodney open http://localhost:8877/library/
